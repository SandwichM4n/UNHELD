...
@export var max_range: float = 200.0  # ← Cut in half (was 400)
@export var cone_angle_degrees: float = 50.0
@export var cooldown_duration: float = 6.0
@export var damage_multiplier: float = 0.5  # Half of basic attack MAX damage
@export var cursor_forgiveness_radius: float = 50.0  # Forgiving cursor targeting
...
func try_activate() -> bool:
	"""Attempt to activate bladesurge. Returns true if successful."""
...
func find_bladesurge_target():
	...
	# PRIORITY 1: Find enemy closest to cursor (within forgiveness radius)
	for enemy in enemies:
		...
		# Find enemy closest to cursor (within forgiveness radius)
		if dist_to_cursor < closest_to_cursor_dist:
			closest_to_cursor_dist = dist_to_cursor
			best_cursor_target = enemy
	
	# If we found an enemy near cursor, ALWAYS use it (cursor has priority!)
	if best_cursor_target:
		return best_cursor_target
	
	# PRIORITY 2: No cursor target? Find enemy in movement direction cone
...
func execute_bladesurge():
	...
	# Calculate dash target (slightly before enemy to avoid overlap)
	...
	var dash_duration = distance / dash_speed  # Time = Distance / Speed
	...
	# Deal damage when we reach target
...
func _on_dash_complete():
	...
	if not is_instance_valid(target_enemy) or target_enemy.is_dead:
		...
		_end_bladesurge(true)  # Enemy died, reset cooldown
		return
	
	# Calculate damage - HALF of MAXIMUM basic attack damage
	var max_basic_damage = 6  # Your max_damage from Damager
	var surge_damage = int(max_basic_damage * damage_multiplier)
	
	# Store enemy HP before damage to check if it dies
	var enemy_hp_before = target_enemy.current_hp
	
	# Deal damage directly
	target_enemy.take_damage(surge_damage, player.global_position)
	
	# Spawn damage number (copy from DamageHandler)
...
func spawn_damage_number(amount: int, pos: Vector2):
	"""Same as DamageHandler - we duplicate to keep it simple for now"""
	var label = Label.new()
	...
	label.z_index = 50
	label.modulate = Color.CYAN  # Different color for bladesurge damage!
